...
def demo_stage_processing():
    ...
    # 技术文档测试
    tech_text = """
    在Python编程中，装饰器（Decorator）是一种设计模式，它允许我们在不修改原函数代码的情况下，
    为函数添加新的功能。装饰器本质上是一个函数，它接受一个函数作为参数，并返回一个新的函数。
    装饰器的主要用途包括：日志记录、性能测试、访问控制和缓存等。在Python中，装饰器使用@符号语法，
    这使得代码更加简洁和易读。装饰器可以嵌套使用，也可以接受参数，这使得它们非常灵活和强大。
    """
    
    print(f"技术文档压缩:")
    print(f"原始长度: {len(tech_text)} 字符")
    print()
    
    result = agent.compress_text(tech_text, compression_ratio=50.0, compression_type=CompressionType.TECHNICAL)
    
    print(f"处理阶段:")
    for stage_name, stage_data in result.processing_stages.items():
        print(f"- {stage_name}: {len(stage_data.get('result', ''))} 字符")
